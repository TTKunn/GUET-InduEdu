...
### 12. tb_application 社团申请表

| 字段名          | 类型                                                | 说明      | 备注                                                 |
| ------------ | ------------------------------------------------- | ------- | -------------------------------------------------- |
| id           | bigint pk auto\_inc                               | 申请主键ID  |                                                    |
| user\_id     | bigint                                            | 申请人用户ID |                                                    |
| club\_id     | bigint                                            | 社团ID    |                                                    |
| type         | enum('join','quit','transfer')                    | 申请类型    | 默认 join（加入）                                        |
| status       | enum('pending','approved','rejected','cancelled') | 审批状态    | 默认 pending（待审批）                                    |
| user\_remark | varchar(255)                                      | 用户备注    | 可空                                                 |
| created\_at  | datetime                                          | 创建时间    | 默认 CURRENT\_TIMESTAMP                              |
| updated\_at  | datetime                                          | 更新时间    | 默认 CURRENT\_TIMESTAMP ON UPDATE CURRENT\_TIMESTAMP |
| deleted\_at  | datetime                                          | 软删除时间   | 可空                                                 |
